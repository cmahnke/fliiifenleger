...
// Extracts the resolved jxl-oxide version from Cargo.lock and exposes it
// to the crate as the JXL_OXIDE_VERSION environment variable (mirrors the
// ultrahdr module's build.rs).
use std::env;
use std::fs;
...
fn main() {
    ...
    println!("cargo:rustc-env=JXL_OXIDE_VERSION={version}");
